...
namespace awsiotsdk {
    namespace samples {
        ...
        ResponseCode SwitchThing::RunSample() {
            ...
            util::String update_topic = "/topic/data";
/*
...
*/
                ...
                // Checking if a member exists
                if (doc[SHADOW_DOCUMENT_STATE_KEY][SHADOW_DOCUMENT_DESIRED_KEY].HasMember(TEMP_KEY)) {
                    // Erasing a member
                    doc[SHADOW_DOCUMENT_STATE_KEY][SHADOW_DOCUMENT_DESIRED_KEY].EraseMember(TEMP_KEY);
                }
#endif
...
#endif
                // Creating JsonValue for key
                util::JsonValue key(TEMP_KEY, doc.GetAllocator());
#ifdef SW_TEMP
                // Adding a member
                if (userInputChar == '1')
...
//		std::cout << payload;
                ...
                util::String payload = "{\"state\":{\"desired\":{\"mytemp\":";
		payload.append(buf);
		payload.append(",\"datetime\":\"");
		payload.append(nowtime);
		payload.append("\"}}}");
...
/*
		char cmd[128];
		sprintf(cmd, "send_data.sh %s", buf);
		system(cmd);
*/

		/*below codes are for update the temperature to cloud */
/*
#define UPDATA_TOPIC "/topic/data"
                util::String p_topic_name_str = UPDATA_TOPIC;
               char payload_t[50];
		strcat(payload_t, "{\"state\": \"");
                strcat(payload_t, buf);
                strcat(payload_t, "\"}");
		util::String payload = payload_t;
                ...
                if (ResponseCode::SUCCESS != rc) {
                    ...
                    AWS_LOG_ERROR(LOG_TAG_SWITCH_SAMPLE,
                                  "Temperature update using publish failed with return code : %d",
                                  static_cast<int>(rc));
                    break;
                 }
*/
